...
房间后可在底部“联机”面板修改当前房间密码，也可确认关闭当前房间，让全部玩家收到
原因并返回主界面；内置服务保持运行，可立即重新开房。旧密码不会回显或写入本地设置；
新玩家立即改用新密码，已经在房间内的玩家不会被断开。加入失败时，表单会持续显示
失败阶段、原因和处理建议，“诊断”页面也会突出显示同一条脱敏结论。
