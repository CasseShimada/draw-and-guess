...
“画猜现场”是一个自带窗口/屏幕采集与房主服务的跨平台桌面派对游戏。0.5.10 提供
三种玩法：
...
1. 房主可先选择固定端口和“仅本机”或“局域网 / 可做端口转发”；直接点击“创建
   房间”时，应用也会按已保存配置自动启动本机服务；
2. 玩家在加入页填写服务器地址 / IP、端口、安全性、房间码、昵称和密码；进入房间后
   昵称会自动加上唯一的 `#四位数字` 后缀；
3. 打开“控制中心 → 采集”，选择外部绘图窗口，调整裁切框并确认；
...
房间码在全局顶部栏紧凑显示且只出现一次，点击即可复制。实际主机进入房间后可在
“控制中心 → 联机”修改当前房间密码，也可在面板底部确认关闭当前房间。其他玩家会
立即看到关闭原因，并在点击确认后返回主界面；房主正常退出程序时也会先广播原因再
停止服务。旧密码不会回显或写入本地设置；新玩家立即改用新密码，已经在房间内的
玩家不会被断开。加入失败时，表单会持续显示失败阶段、原因和处理建议，“控制中心
→ 诊断”也会突出显示同一条脱敏结论。

游戏进行中，逻辑房主可从“当前阶段操作栏”进入仅本地可见的本局设置页。打开页面
不会发送返回大厅命令，不会暂停服务器计时，也不会改变其他玩家的画面。房主可以
放弃草稿直接返回游戏，或确认“应用并重启游戏”：服务器会原子清理旧计时器、旧帧、
旧采集授权和本局进度，应用新参数并在不移除任何成员的前提下开始同模式新一局。
词库、参考图、房间密码、聊天、昵称和头像保持不变；切换到不同模式仍会进入目标
模式的真实大厅，不会自动开局。
...
设置页可重新检查权限，并提供默认关闭的开机启动、托盘常驻和固定事件通知选项。
关闭窗口默认退出整个应用；只有主动开启“保持在托盘运行”后才会隐藏到托盘。
...
协议版本为 `5`。v5 新增带 `modeSessionId`、完整模式设置、`commandId` 和可选部分
回放处理方式的原子 `game:restart` 命令；v4 客户端会在连接边界被明确拒绝。图片
二进制头继续沿用固定宽度：
...
packages/
  ...
  protocol/      协议 v5 schema、消息与二进制帧编解码
...
自动化测试覆盖 v5 schema/426、连接目标解析、设置 v6 迁移、精确端口绑定、连接预检、
单端口 TCP/反向代理、随机与持久昵称、三模式状态机、模式切换、Pass、经典模式私密
选词顺序、fake-clock 暂停、
...
`docs/release-verification-0.5.10.md`；没有实机运行的平台或双机网络矩阵不会标记为
已验证。
