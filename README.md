...
“画猜现场”是一个自带窗口/屏幕采集与房主服务的跨平台桌面派对游戏。0.5.9 提供
三种玩法：
...
房间码紧凑显示在大厅左上角，点击即可复制，不再占用一整条大厅横幅。实际主机进入
房间后可在顶部“联机”面板修改当前房间密码，也可确认关闭当前房间。其他玩家会立即
看到关闭原因，并在点击确认后返回主界面；房主正常退出程序时也会先广播原因再停止
服务。旧密码不会回显或写入本地设置；新玩家立即改用新密码，已经在房间内的玩家不会
被断开。加入失败时，表单会持续显示失败阶段、原因和处理建议，“诊断”页面也会突出
显示同一条脱敏结论。

游戏进行中，逻辑房主可点击顶部“返回房间”进入仅本地可见的房间设置页。打开页面
...
`docs/release-verification-0.5.9.md`；没有实机运行的平台或双机网络矩阵不会标记为
已验证。
